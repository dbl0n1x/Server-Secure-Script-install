...
# Спрашиваем, нужно ли вообще ограничивать SSH по доверенному IP
TRUSTED_IP=""
read -p "Ограничить SSH доверенным IP? (y/N): " RESTRICT_IP
...
if [[ "$RESTRICT_IP" =~ ^[Yy]$ ]]; then
    ...
    success "Доступ по SSH будет ограничен IP: $TRUSTED_IP"
else
    warn "Ограничение SSH по IP отключено — вход разрешён с любого адреса (только по ключу)"
fi
...
# ════════════════════════════════════════════════════════════
# 3. Ограничение SSH по IP (только если задан доверенный IP)
# ════════════════════════════════════════════════════════════
...
EOF
    ...
    info "Пропускаем настройку hosts.allow / hosts.deny (доверенный IP не задан)"
fi
...
# Собираем список ignoreip: localhost всегда, доверенный IP — если задан
IGNORE_IP="127.0.0.1/8"
[[ -n "$TRUSTED_IP" ]] && IGNORE_IP="$IGNORE_IP $TRUSTED_IP"
...
if [[ -n "$TRUSTED_IP" ]]; then
    ...
    echo -e "  ${GREEN}✓${NC} hosts.allow/deny настроены (trusted: $TRUSTED_IP)"
else
    echo -e "  ${GREEN}✓${NC} SSH: только ключ, с любого IP, порт $SSH_PORT"
    echo -e "  ${YELLOW}–${NC} hosts.allow/deny не трогали (доверенный IP не задан)"
fi
echo -e "  ${GREEN}✓${NC} Fail2Ban: ban на 1ч после 3 попыток (ignoreip: $IGNORE_IP)"
